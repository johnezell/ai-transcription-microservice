...
class MusicTermController extends Controller
{
    ...
    {
        $categories = MusicTermCategory::with(['terms' => function($query) {
            $query->orderBy('term');
        }])
        ...
        return Inertia::render('Admin/MusicTerms/Index', [
    ...
    {
        return Inertia::render('Admin/MusicTerms/CreateCategory');
    }
    ...
    public function storeCategory(Request $request)
    {
        ...
        while (MusicTermCategory::where('slug', $validated['slug'])->exists()) {
            $count++;
            $validated['slug'] = $originalSlug . '-' . $count;
        }
        
        $validated['active'] = true;
        $validated['display_order'] = MusicTermCategory::max('display_order') + 1;
        
        $category = MusicTermCategory::create($validated);
        
        return redirect()->route('admin.music-terms.index')
    ...
    {
        $category = MusicTermCategory::findOrFail($id);
        
        return Inertia::render('Admin/MusicTerms/EditCategory', [
            'category' => $category
        ]);
    }
    ...
    public function updateCategory(Request $request, string $id)
    {
        ...
        if ($request->name !== $category->name) {
            ...
            $count = 0;
            $originalSlug = $validated['slug'];
            while (MusicTermCategory::where('slug', $validated['slug'])
                ->where('id', '!=', $id)
                ->exists()) {
                $count++;
                $validated['slug'] = $originalSlug . '-' . $count;
    ...
    {
        $categories = MusicTermCategory::where('active', true)
            ->orderBy('display_order')
            ->orderBy('name')
            ->get();
            
        return Inertia::render('Admin/MusicTerms/CreateTerm', [
            'categories' => $categories
        ]);
    }
    ...
    {
        $validated = $request->validate([
            'category_id' => 'required|exists:music_term_categories,id',
            'term' => [
                'required',
                'string',
                'max:255',
                function ($attribute, $value, $fail) use ($request) {
                    ...
                    $exists = MusicTerm::where('category_id', $request->category_id)
                        ->where('term', $value)
                        ->exists();
        ...
        $term = MusicTerm::create($validated);
    ...
    /**
     * Export all music terms as JSON.
     */
    ...
    {
        return Inertia::render('Admin/MusicTerms/Import');
    }
    ...
    public function import(Request $request)
    {
        ...
        try {
            ...
            $displayOrder = MusicTermCategory::max('display_order') ?? 0;
            ...
            foreach ($jsonData as $categorySlug => $terms) {
                ...
                $category = MusicTermCategory::firstOrNew(['slug' => $categorySlug]);
