...
class LocalTruefireSegment extends Model
{
    use HasFactory;
     ...
     * @var string
     */
    protected $table = 'local_truefire_segments';
    ...
    {
        return $this->belongsTo(LocalTruefireChannel::class, 'channel_id', 'id');
    }
    ...
    /**
     * Generate a signed URL for this segment's video.
     * This method maintains compatibility with the original Segment model.
     *
    ...
    public function getSignedUrl($expirationSeconds = 518400)
    {
        try {
            ...
            \Log::info('Generating S3 signed URL for local segment', [
                'segment_id' => $this->id,
            ...
            \Log::info('Successfully generated S3 signed URL for local segment', [
                'segment_id' => $this->id,
                's3_key' => $video,
            ...
            \Log::error('Failed to generate S3 signed URL for local segment', [
                'segment_id' => $this->id,
                'video' => $this->video,
    ...
    private function validateAwsCredentials(): void
    {
        ...
        \Log::debug('AWS credentials validation passed for local segment', [
    ...
    /**
     * Get the S3 path for this segment's video file.
     * This method maintains compatibility with the original Segment model.
     *
     * @return string
     */
     ...
     * Prioritizes 'name' field, falls back to 'subhead', then generates a default.
     */
    public function getTitleAttribute()
    {
        return $this->name ?: ($this->subhead ?: "Segment #{$this->id}");
    }
}