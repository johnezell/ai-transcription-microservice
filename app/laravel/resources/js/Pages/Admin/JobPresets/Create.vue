...
const configuration = {
    transcription: {
        initial_prompt: '',
        temperature: props.transcriptionOptions?.advanced?.temperature?.default || 0,
        word_timestamps: props.transcriptionOptions?.advanced?.word_timestamps?.default || true,
        condition_on_previous_text: props.transcriptionOptions?.advanced?.condition_on_previous_text?.default || false,
        beam_size: props.transcriptionOptions?.advanced?.beam_size?.default || 5,
        compression_ratio_threshold: props.transcriptionOptions?.advanced?.compression_ratio_threshold?.default || 2.4,
        logprob_threshold: props.transcriptionOptions?.advanced?.logprob_threshold?.default || -1.0,
        no_speech_threshold: props.transcriptionOptions?.advanced?.no_speech_threshold?.default || 0.6,
    },
    audio: {
        ...
        noise_reduction: props.audioExtractionOptions?.advanced?.noise_reduction?.default || false,
        normalize_audio: props.audioExtractionOptions?.advanced?.normalize_audio?.default || false,
        volume_boost: props.audioExtractionOptions?.advanced?.volume_boost?.default || 0,
        low_pass: props.audioExtractionOptions?.advanced?.low_pass?.default || null,
        high_pass: props.audioExtractionOptions?.advanced?.high_pass?.default || null,
    },
    terminology: {
...
const form = useForm({
    name: '',
    description: '',
    model: props.transcriptionOptions?.common?.model_name?.default || 'base',
    language: props.transcriptionOptions?.common?.language?.default || '',
...
const submit = () => {
    form.post(route('admin.job-presets.store'));
};

// Helper function to determine if we have transcription options from the service
...
// Helper function to determine if we have audio extraction options from the service
...
<template>
    <Head title="Create Transcription Preset" />

    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Create Transcription Preset</h2>
                        ...
                        <template v-slot:default="{ activeTab }">
                            ...
                            <div v-if="activeTab === 'general'">
                                ...
                                <Card title="Status Settings">
                                    ...
                                    <div class="mb-2 flex items-center">
                                        <Checkbox id="is_default" v-model:checked="form.is_default" name="is_default" />
                            ...
                            <div v-if="activeTab === 'transcription'">
                                ...
                                <Card title="Advanced Transcription Options">
                                    ...
                                    <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <!-- Additional advanced options if available -->
