...
class GuitarTerminologyEvaluator:
    ...
    def _update_json_with_new_confidence(self, original_json: Dict[str, Any], 
        ...
        # Create a lookup dictionary for enhanced segments by (word, start_time, end_time)
        enhanced_lookup = {}
        for seg in segments:
            # Use word + timing as key for precise matching
            key = (seg.word, round(seg.start, 3), round(seg.end, 3))
            enhanced_lookup[key] = seg
        
        # Helper function to find matching enhanced segment
        def find_enhanced_segment(word_data):
            ...
            if key in enhanced_lookup:
                return enhanced_lookup[key]
            
            # Try fuzzy timing match (within 0.1 seconds)
            for (w, s, e), seg in enhanced_lookup.items():
                if (w == word and 
                    abs(s - start) < 0.1 and 
                    abs(e - end) < 0.1):
                    return seg
                ...
                if enhanced_seg:
                    ...
                    if enhanced_seg.original_confidence != enhanced_seg.confidence:
                        ...
                        for term in evaluated_terms:
                            if (term.get('word') == enhanced_seg.word and 
                                abs(term.get('start', 0) - enhanced_seg.start) < 0.1 and 
                                abs(term.get('end', 0) - enhanced_seg.end) < 0.1):
                        ...
                        word_data['normalized_form'] = self._normalize_word(enhanced_seg.word)
    ...
    def _apply_counting_pattern_boosts(self, segments: List[WordSegment], patterns: List[MusicalCountingPattern]) -> int:
        ...
        # Define compound musical term pattern types that should always be boosted
        compound_musical_patterns = {
            "roman_numeral_chord", 
            "extended_chord_notation", 
            "altered_chord_notation"
        }
        
        for pattern in patterns:
            logger.info(f"Applying musical counting pattern boost: {pattern.description}")
            
            # Check if this is a compound musical term
            is_compound_musical_term = pattern.pattern_type in compound_musical_patterns
            ...
            for i in range(pattern.start_index, pattern.end_index + 1):
                if i < len(segments):
                    ...
                    # For compound musical terms, always boost regardless of threshold
                    # For other patterns, only boost if current confidence is below threshold
                    should_boost = is_compound_musical_term or segment.confidence < self.confidence_threshold
                    ...
                    if should_boost:
                        ...
                        boost_reason = "compound_musical_term" if is_compound_musical_term else "musical_counting_pattern"
                        logger.debug(f"Boosted musical counting word '{segment.word}': "
                                   f"{segment.original_confidence:.3f} -> {segment.confidence:.3f} "
                                   f"(pattern: {pattern.pattern_type}, reason: {boost_reason})")
